...
int Admin::add_user(string role){
    ...
    sql_query = conn.query("INSERT INTO users (user_name, admission_number, email_address, password, role, First_name, Last_name) VALUES ('" + user + "', '" + admission_number + "', '" + email_address + "', '" + bcrypt::generateHash(pass) + "', '" + role + "', '" + first_name + "', '" + last_name + "');");
    if(sql_query){
        return 1;
    }else{
